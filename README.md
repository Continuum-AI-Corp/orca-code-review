...
| `precision-filter` | `true` | Precision post-processing between the engine and the merge gate. `"true"` runs two layers on the raw engine findings before the gate: (L1) a deterministic filter that git-greps each finding's own `existing_code` snippet against the reviewed commit and re-homes or drops findings mis-filed onto files that do not contain the referenced code (the "same finding copy-pasted across unrelated files" FP pattern seen on large PRs), then (L2) a single-call LLM judge from an **independent** vendor that clusters findings by root cause and drops speculative / low-value ones below a confidence threshold. Set `"false"` to post the engine's raw findings directly (useful for debugging a regression). Both layers are soft-fail — an error in either keeps the prior stage's findings and never aborts the review. |
| `judge-model` | `deepseek/deepseek-v4-pro` | Model used by the L2 judge stage of `precision-filter`. Should be **different from the reviewer model** — independence is the whole point. Cheap; single request per pass. Ignored when `precision-filter` is `"false"`. |
| `judge-threshold` | `0.7` | Keep-threshold for the L2 judge's per-cluster confidence score (0–1). Findings with confidence below this are dropped. Default keeps high-certainty clusters and drops speculative ones without cutting P2s as a class. Lower to keep more; raise to be stricter. |
